package com.stock.intrinio.sentiment
...
object SentimentAnalyzer {
  ...
  private def extractSentiments(text: String): List[(String, Sentiment)] = {
    ...
    val res = sentences
      .toArray()
      .map(x => x.asInstanceOf[CoreMap])
      .map(sentence => (sentence, sentence.get(classOf[SentimentCoreAnnotations.SentimentAnnotatedTree])))
      .map { case (sentence, tree) => (sentence.toString,
        SentimentParser.toSentiment(RNNCoreAnnotations.getPredictedClass(tree))) }
      .toList
    res
  }
}
